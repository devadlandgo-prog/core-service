...
public interface ListingDraftRepository extends JpaRepository<ListingDraft, UUID> {
    @Query("SELECT d FROM ListingDraft d WHERE d.id = :id AND d.ownerId = :ownerId AND d.deleted = false")
    Optional<ListingDraft> findByIdAndOwnerId(@Param("id") UUID id, @Param("ownerId") UUID ownerId);

    @Query("SELECT d FROM ListingDraft d WHERE d.ownerId = :ownerId AND d.status != 'PUBLISHED' AND d.deleted = false ORDER BY d.updatedAt DESC")
